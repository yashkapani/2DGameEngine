...
void Unittest()
{
	...
	MessagedAssert(h1 == h2, "");
	Engine::NamedBitSet<char> NBS;
	unsigned int n1 = NBS.GetBitIndex(h1);
	unsigned int n2 = NBS.GetBitIndex(h2);
	unsigned int n3 = NBS.GetBitIndex(h3);

	MessagedAssert(n1 == n3, "");
	bool res = NBS.FindBitIndex("test2", n2);
	...
	const char * SP1 = pTestPool->find("This");
	const char * SP2 = pTestPool->find("is");
	const char * SP3 = pTestPool->find("a");
	const char * SP4 = pTestPool->find("test");
	const char * SP5 = pTestPool->find("a");

	MessagedAssert(SP5 == SP3,"");
