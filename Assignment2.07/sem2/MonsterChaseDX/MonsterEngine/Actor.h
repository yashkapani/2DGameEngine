...
struct BoundingBox
{
	...
	Vector3 Extents = Vector3(25.0f, 25.0f, 25.0f); //X,Y,Z half-extents.
};

	class Actor
	{
	private:
		Vector3  m_Acceleration, m_Rotation;
	...
	public:
		...
		Actor(const Vector3 & i_InitialPosition, const Vector3 & i_InitialVelocity, const Vector3 & i_InitialRotation, const char* p_name) :
			m_bIsDead(false),
		...
		const Vector3 & GetBoundingBoxCenter() const { return m_CollisionBox.Center; }
		const Vector3 & GetBoundingBoxExtent() const { return m_CollisionBox.Extents; }
