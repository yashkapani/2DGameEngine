...
int main(int i_argc, char ** i_argl)
{
	...
	RingBuffer<Vector3> ringBuffer = RingBuffer<Vector3>(5); // max 5 frames of data going back
	cout << "Maximum Possible Size Positions stored in the Ring Buffer  = " << ringBuffer.size() << endl;
...
bck:
	...
	do
	{
		...
		Vector3 mposition = Player->GetPosition();

		cout << mposition.X();
		ringBuffer.add(mposition);
		for (int i = 0; i < n; i++)
		{
		...
		cout << "RingBuffer Unit Test" << endl;
		for (int i = 0; i < ringBuffer.count(); i++)
		{
			cout << "Position " << ringBuffer.count() - i - 1 << " : ";
			cout << " = X : " << ringBuffer[i].X() << " Y : " << ringBuffer[i].Y() << " Z : " << ringBuffer[i].Z() << endl;
		}
		cin >> ch;
