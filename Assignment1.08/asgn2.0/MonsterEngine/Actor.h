...
#include <malloc.h>	
	...
	public:
		...
		{
			DEBUG_PRINT(4,"Calling new( size_t ) with size %d line: %d file: &s .\n", i_size, __LINE__, __FILE__);
		...
		{
			DEBUG_PRINT(3,"Calling delete( void * ) on %p.\n", i_ptr);
			MessagedAssert(s_pAllocator, "s_pAllocator should be NULL."__FILE__, __LINE__);
			//assert(s_pAllocator);
